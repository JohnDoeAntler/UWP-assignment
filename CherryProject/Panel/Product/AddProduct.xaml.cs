...
namespace CherryProject.Panel.Product
{
	...
	public sealed partial class AddProduct : Page
	{
		...
		private bool IsDoubleNumeric(string str) => double.TryParse(str, out _) || string.IsNullOrEmpty(str);
		private bool IsIntegerNumeric(string str) => int.TryParse(str, out _) || string.IsNullOrEmpty(str);

		private void Price_KeyDown(object sender, TextBoxBeforeTextChangingEventArgs e) => e.Cancel = !IsDoubleNumeric(e.NewText);

		private void Weight_KeyDown(object sender, TextBoxBeforeTextChangingEventArgs e) => e.Cancel = !IsDoubleNumeric(e.NewText);

		private void DangerLevel_KeyDown(object sender, TextBoxBeforeTextChangingEventArgs e) => e.Cancel = !IsIntegerNumeric(e.NewText);
			...
			if (result == ContentDialogResult.Primary)
			{
				...
				else
				{
					try
					{
						...
						{
							var p = await context.Products.AddAsync(
								new Products()
								{
									Name = Name.Text,
